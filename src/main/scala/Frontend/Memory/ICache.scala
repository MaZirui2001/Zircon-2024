...
class I_Stage2_Signal extends I_Stage1_Signal {
    val rtag        = UInt(l1_tag.W)
    val rdata       = UInt(l1_line_bits.W)
    val hit         = Bool()
    val lru         = UInt(2.W)
    val paddr       = UInt(32.W)
    val uncache     = Bool()
...
class I_Pipeline_IO extends Bundle {
    ...
    val rdata       = Output(UInt(l1_line_bits.W))
...
class ICache extends Module {
    ...
    */
    ...
    val data_tab    = VecInit.fill(l1_way)(Module(new xilinx_single_port_ram_read_first(l1_line_bits, l1_index_num)).io)
    ...
    io.pp.miss          := miss_c1
    io.pp.rdata         := Mux(c1s3.uncache, rbuf(31, 0), (Mux1H(fsm.io.cc.r1H, VecInit(Mux1H(c1s3.hit, c1s3.rdata), rbuf)).asTypeOf(Vec(l1_line / (4*nfetch), UInt((32*nfetch).W))))(offset(c1s3.vaddr) >> fetch_offset) >> (offset(c1s3.vaddr)(fetch_offset-1, 2) << 5)) 
